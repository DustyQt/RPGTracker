...
class Character:
    ...
    def load_base_stats(self, character):
        character['equipment'] = self.equipment_controller.get_character_equipment(character['equipment'])
        character['basic_stats']= self.calculate_stats(self.get_basic_stats(), character)
        return character
    ...
    def get_class_stats(self, character):
        ...
        for stat in main_class.get('stats'):
            utils.sum_dict(class_stats, stat, main_class.get('stats').get(stat))
        return class_stats
        ...
        for char_pasive_plain in char_class_pasives_plain:
            for effect in char_pasive_plain['effects']:
                if effect.get('effect') == 'PASSIVE':
                    ...
                    utils.sum_dict(sum_class_pasives_plain, stat, self.calculate(character, formula, dv))
        for char_pasive_percentage in char_class_pasives_percentage:
            for effect in char_pasive_percentage['effects']:
                if effect.get('effect') == 'PASSIVE':
                    ...
                    formula = effect['formula']
                    dv = effect['default_values']
                    utils.sum_dict(sum_class_pasives_percentage, stat, self.calculate(character, formula, dv))
        ...
        for fit in fit_passives_plain: 
            for effect in fit['effects']:
                if effect.get('effect') == 'PASSIVE':
                    ...
                    formula = effect['formula']
                    dv = effect['default_values']
                    utils.sum_dict(sum_fits_passives_plain, stat, self.calculate(character, formula, dv))
        for fit in fit_passives_percentage: 
            for effect in fit['effects']:
                if effect.get('effect') == 'PASSIVE':
                    ...
                    formula = effect['formula']
                    dv = effect['default_values']
                    utils.sum_dict(sum_fits_pasives_percentage, stat, self.calculate(character, formula, dv))
